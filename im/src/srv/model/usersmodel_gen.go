...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Id, data.Avatar, data.Username, data.Mobile, data.Password, data.Status, data.Gender)
	}, usersIdKey, usersMobileKey)
	return ret, err
}
...
func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.Avatar, newData.Username, newData.Mobile, newData.Password, newData.Status, newData.Gender, newData.Id)
	}, usersIdKey, usersMobileKey)
	return err
}
...
func (m *defaultUsersModel) FindByIds(ctx context.Context,ids []string)([]*Users,error){
	...
	res:=[]*Users{}
	err:=m.QueryRowNoCacheCtx(ctx,&res,query,ids)
	switch err{
	case nil:
		return res,nil
	default:
...
func (m *defaultUsersModel) FindByName(ctx context.Context,name string)([]*Users,error){
	query:=fmt.Sprintf("select %s from %s where `username` like ?",usersRows,m.table)
	res:=[]*Users{}
	err:=m.QueryRowNoCacheCtx(ctx,&res,query,name)
	switch err{
	case nil:
		return res,nil
	default:
