...
// 用于记录一件商品的扣除回滚记录
type InventoryDecrRecord struct {
	GoodsId  uint32 `json:"goods_id"`
	GoodsNum int32  `json:"goods_num"`
}

type InventoryDecrRecordList []*InventoryDecrRecord

func (g InventoryDecrRecordList) Value() (driver.Value, error) {
	return json.Marshal(g)
}
func (g *InventoryDecrRecordList) Scan(value interface{}) error {
	return json.Unmarshal(value.([]byte), &g)
}

// 用于库存扣减回滚的记录,即在微服务层记录一次库存的扣减,
// 如果事务出现问题能根据Record恢复
type OrderDecrRecord struct {
	//订单编号
	OrderSign string `gorm:"column:order_sign;type:varchar(50)"`
	//当前库存状态,用int类型能实现幂等性
	Status      int32                   `gorm:"type:int"`
	GoodsRecord InventoryDecrRecordList `gorm:"type:varchar(4000)"`
}
